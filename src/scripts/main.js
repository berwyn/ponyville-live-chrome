...
function __debug(msg) {
	if (__debug_mode)
		console.log("[DEBUG][" + arguments.callee.caller.name + "] " + msg);
}
/**************/
...
function pvl_load_stations() {
	$.getJSON('http://ponyvillelive.com/api/station/list/category/audio/client/pvldesktopapp/guid/' + __debug_guid, function(data) {
		if (data.status != "success") {
			...
			pvl_error("Failed to connect to PVL!", "Please check your internet and try again. " + __debug_guid + ":" + data.status + ":" + data.error, false);
			return;
		}
...
function pvl_update_stations() {
	$.getJSON('http://ponyvillelive.com/static/api/nowplaying.json', function(data) {
		...
		for (station_id in data) {
			...
			station_li = $('li[id="station_' + station_data.code + '"]');
			...
			station_li.find('h4 > .nowplaying-info > .nowplaying-listeners').html('<i class="fa fa-user"></i>&nbsp;' + station_data.listeners);
			station_li.find('h4 > .nowplaying-info > .nowplaying-listeners').attr('title', station_data.listeners + ' Listeners');

			station_li.find('.nowplaying-song').text(station_data.title);
			station_li.find('.nowplaying-song').attr('title', station_data.title);

			station_li.find('.nowplaying-artist').text(station_data.artist);
			station_li.find(' .nowplaying-artist').attr('title', station_data.artist);
			...
			if (station_data.id == $('.display > .station').attr('data-station-id')) {
				...
				display_station.find('.info > h1').text(station_data.name);
				display_station.find('.info > h1').append('<span class="nowplaying-info"><span class="nowplaying-listeners"></span></span>');
				display_station.find('.info > .genre-info').text(station_data.genre);

				display_nowplaying.find('.panel-top > h4 > .nowplaying-station-listeners').text(station_data.listeners + ' Listeners Tuned In');

				if (_last_played_song != station_data.song_sh_id) {
					display_nowplaying.find('.nowplaying-info > .nowplaying-song').text(station_data.title);
					display_nowplaying.find('.nowplaying-info > .nowplaying-song').attr('title', station_data.title);

					display_nowplaying.find('.nowplaying-info > .nowplaying-artist').text(station_data.artist);
					display_nowplaying.find('.nowplaying-info > .nowplaying-artist').attr('title', station_data.artist);

					display_controls.find('.vote .nowplaying-score').attr('data-id', station_data.song_sh_id);
					display_controls.find('.vote .nowplaying-score').attr('data-orginal', station_data.song_score);
					display_controls.find('.vote .nowplaying-score').text(station_data.song_score);
